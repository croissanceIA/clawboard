...
export function frequencyLabel(expr: string): string {
  const patterns: [RegExp, string | ((m: RegExpMatchArray) => string)][] = [
    ...
    [/^0 ([\d,]+) \* \* \*$/, (m) => `Quotidien à ${m[1].split(',').map(h => `${h}h`).join(', ')}`],
    [/^(\d+) ([\d,]+) \* \* \*$/, (m) => `Quotidien à ${m[2].split(',').map(h => `${h}h${pad(m[1])}`).join(', ')}`],
    [/^0 ([\d,]+) \* \* 1-5$/, (m) => `Lun-Ven à ${m[1].split(',').map(h => `${h}h`).join(', ')}`],
    [/^(\d+) ([\d,]+) \* \* 1-5$/, (m) => `Lun-Ven à ${m[2].split(',').map(h => `${h}h${pad(m[1])}`).join(', ')}`],
