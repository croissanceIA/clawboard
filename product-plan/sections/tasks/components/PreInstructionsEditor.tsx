...
import type { PreInstruction } from '../types'
  ...
  return (
    <div className="mx-auto max-w-2xl">
      <div className="mb-5 flex items-start gap-2.5 rounded-xl border border-blue-100 bg-blue-50/50 px-4 py-3 dark:border-blue-900/40 dark:bg-blue-950/20">
        ...
        <p className="text-xs leading-relaxed text-blue-700 dark:text-blue-300">These instructions are prepended to every task sent to the agent. They support line breaks and Markdown formatting.</p>
      </div>
      ...
      <div className="mt-4 flex items-center justify-between">
        <span className="text-[11px] text-zinc-400 dark:text-zinc-500">Last saved: {new Date(preInstruction.updatedAt).toLocaleDateString('fr-FR', { day: '2-digit', month: 'long', year: 'numeric', hour: '2-digit', minute: '2-digit' })}</span>
        <button onClick={handleSave} disabled={!isDirty && !saved} className={`inline-flex items-center gap-1.5 rounded-lg px-4 py-2 text-xs font-medium shadow-sm transition-all ${saved ? 'bg-emerald-600 text-white' : isDirty ? 'bg-blue-600 text-white hover:bg-blue-700 dark:bg-blue-500 dark:hover:bg-blue-600' : 'bg-zinc-100 text-zinc-400 cursor-not-allowed dark:bg-zinc-800 dark:text-zinc-500'}`}>{saved ? <><Check className="size-3.5" />Saved</> : <><Save className="size-3.5" />Save</>}</button>
      </div>
    </div>
  )
}
