...
import type { CronJob, Template, KanbanColumn } from '../types'
  ...
  return (
    <div>
      <div className="mb-5 flex flex-wrap gap-1.5">
        {[{ id: 'all' as const, label: 'All' }, ...COLUMNS.map((c) => ({ id: c.id, label: c.label }))].map((f) => (
                  ...
                  return (
                    <button key={job.id} onClick={() => onViewTaskDetail?.(job.id)} className={`group w-full rounded-xl border p-3.5 text-left transition-all hover:shadow-md hover:-translate-y-0.5 ${accent.border} bg-white dark:bg-zinc-900`}>
                      <div className="flex items-start justify-between gap-2">
                        <p className="text-sm font-medium text-zinc-900 dark:text-zinc-100 leading-snug">{tpl?.name ?? job.id}</p>
                      ...
                      {!tpl && <span className="mt-2 inline-flex items-center gap-1 rounded-md bg-amber-50 px-2 py-0.5 text-[11px] font-medium text-amber-600 dark:bg-amber-950/50 dark:text-amber-400"><AlertTriangle className="size-3" />No template</span>}
                      ...
                      {job.consecutiveErrors > 0 && <div className="mt-2 flex items-center gap-1 text-[11px] text-red-500"><AlertTriangle className="size-3" />{job.consecutiveErrors} consecutive error{job.consecutiveErrors > 1 ? 's' : ''}</div>}
                    </button>
                  )
                })}
                {jobs.length === 0 && <div className="rounded-xl border border-dashed border-zinc-200 px-4 py-8 text-center text-xs text-zinc-400 dark:border-zinc-800 dark:text-zinc-500">No tasks</div>}
              </div>
            </div>
          )
        })}
