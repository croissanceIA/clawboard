...
import type { ExecutionLog, Template } from '../types'
  ...
  return (
    <div>
      <div className="mb-5 flex flex-col gap-3 sm:flex-row sm:items-center">
        <div className="relative flex-1"><Search className="pointer-events-none absolute left-3 top-1/2 size-4 -translate-y-1/2 text-zinc-400 dark:text-zinc-500" /><input value={search} onChange={(e) => handleSearch(e.target.value)} placeholder="Search executions..." className="w-full rounded-lg border border-zinc-200 bg-white py-2 pl-9 pr-4 text-sm text-zinc-900 placeholder:text-zinc-400 focus:border-blue-300 focus:outline-none focus:ring-2 focus:ring-blue-100 dark:border-zinc-800 dark:bg-zinc-900 dark:text-zinc-100 dark:placeholder:text-zinc-500 dark:focus:border-blue-700 dark:focus:ring-blue-950/50" /></div>
        <select value={filterTemplate} onChange={(e) => handleTemplateFilter(e.target.value)} className="rounded-lg border border-zinc-200 bg-white px-3 py-2 text-sm text-zinc-700 focus:border-blue-300 focus:outline-none dark:border-zinc-800 dark:bg-zinc-900 dark:text-zinc-300"><option value="">All templates</option>{templates.map((t) => <option key={t.id} value={t.id}>{t.name}</option>)}</select>
      </div>
      <div className="rounded-xl border border-zinc-200 bg-white dark:border-zinc-800 dark:bg-zinc-900">
        ...
        <div className="archive-grid max-md:!hidden border-b border-zinc-100 px-4 py-2.5 dark:border-zinc-800">
          <span className="text-[11px] font-semibold uppercase tracking-wider text-zinc-400 dark:text-zinc-500">Execution</span>
          <span className="text-[11px] font-semibold uppercase tracking-wider text-zinc-400 dark:text-zinc-500">Date</span>
          <span className="text-[11px] font-semibold uppercase tracking-wider text-zinc-400 dark:text-zinc-500">Status</span>
          <span className="text-[11px] font-semibold uppercase tracking-wider text-zinc-400 dark:text-zinc-500">Duration</span>
          <span className="text-[11px] font-semibold uppercase tracking-wider text-zinc-400 dark:text-zinc-500">Cost</span>
        </div>
        <div className="divide-y divide-zinc-50 dark:divide-zinc-800/50">
            ...
            return (
              <div key={log.id}>
                <button onClick={() => setExpandedId(isExpanded ? null : log.id)} className="archive-grid w-full items-center px-4 py-3 text-left transition-colors hover:bg-zinc-50 dark:hover:bg-zinc-800/30">
                  <div className="flex items-center gap-2 md:gap-3 min-w-0">{isExpanded ? <ChevronDown className="size-3.5 shrink-0 text-zinc-400" /> : <ChevronRight className="size-3.5 shrink-0 text-zinc-400" />}<div className="min-w-0"><p className="truncate text-sm font-medium text-zinc-900 dark:text-zinc-100">{tpl?.name ?? log.cronJobId ?? 'Unknown'}</p><p className="truncate text-[11px] text-zinc-400 md:hidden dark:text-zinc-500">{new Date(log.executedAt).toLocaleString('fr-FR', { day: '2-digit', month: '2-digit', hour: '2-digit', minute: '2-digit' })}</p></div></div>
                ...
                {isExpanded && (
                  <div className="border-t border-zinc-100 bg-zinc-50/50 px-4 py-4 dark:border-zinc-800 dark:bg-zinc-950/30">
                    ...
                    {log.promptTokens !== null && <div className="mb-3 flex flex-wrap gap-3 text-[11px] text-zinc-400 dark:text-zinc-500"><span>Model: <span className="font-medium text-zinc-600 dark:text-zinc-300">{log.model}</span></span><span>Prompt: <span className="font-medium text-zinc-600 dark:text-zinc-300">{log.promptTokens?.toLocaleString()}</span></span><span>Completion: <span className="font-medium text-zinc-600 dark:text-zinc-300">{log.completionTokens?.toLocaleString()}</span></span></div>}
                    <div className="relative">
                      <pre className="max-h-48 overflow-y-auto rounded-lg border border-zinc-200 bg-white p-3 text-xs leading-relaxed text-zinc-700 dark:border-zinc-700 dark:bg-zinc-900 dark:text-zinc-300" style={{ fontFamily: "'JetBrains Mono', monospace" }}>{logOutput || 'No output.'}</pre>
                      {logOutput && <button onClick={() => handleCopy(logOutput, log.id)} className="absolute right-2 top-2 rounded-md border border-zinc-200 bg-white p-1.5 text-zinc-400 transition-colors hover:bg-zinc-50 hover:text-zinc-600 dark:border-zinc-700 dark:bg-zinc-800 dark:hover:bg-zinc-700 dark:hover:text-zinc-300" title="Copy to clipboard">{copiedId === log.id ? <Check className="size-3.5 text-emerald-500" /> : <Copy className="size-3.5" />}</button>}
                    </div>
                  </div>
                )}
              </div>
          ...
          {filteredLogs.length === 0 && <div className="py-12 text-center text-sm text-zinc-400 dark:text-zinc-500">{search || filterTemplate ? 'No executions match your filters.' : 'No archived executions yet.'}</div>}
        </div>
      </div>
    </div>
  )
